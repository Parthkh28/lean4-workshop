...
│   ├── satirical/       # Why specifications matter
│   │   ├── 01_O1Sort.lean       # O(1) sort with weak permutation spec
│   │   ├── 02_O1Search.lean     # O(1) search with weak element spec
│   │   ├── 03_O1Hash.lean       # O(1) hash with weak uniqueness spec
│   │   ├── 04_O1Compress.lean   # O(1) compress with weak size spec
│   │   └── 05_O1Encrypt.lean    # O(1) encrypt with weak reversibility spec
│   └── correct/         # How specifications should be written
│       └── 01_CorrectSort.lean  # Proper sort with permutation proof
